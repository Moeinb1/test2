...
const createWithOpacityColor = (color) => {
  let colors = {};
  const opacities = [
    { value: '0D', name: 5 },
    { value: '1A', name: 10 },
...
const getCssVarByName = (name) => `var(--arzesh-colors-${name})`;
...
const createThemeColorMode = (name, props, mode) => {
  let tempLight = props.theme.colors;
  let tempDark = props.theme.colors.dark;
  const key = `--arzesh-colors-${name}`;
...
const createRootObject = (props, mode) => {
  let temp = {};
...
const getCSSVarFromColorName = (chakraColors, name = '') => {
  ...
  Object.entries(chakraColors).forEach(([key, value]) => {
    if (typeof value === 'object') {
      ...
      tailwindColors[key] = `var(--arzesh-colors-${
        name ? name + '-' : ''
      }${key})`;
    }
  });
